...
class CrowdMaterial extends THREE.ShaderMaterial {
	...
	// updateMap(path0){
	// 	let path="./assets/textures/"+path0
    //     var scope=this;
    ...
    //             // texture.flipY = false;
	// 			scope.map=texture
	// 			scope.map.needsUpdate=true
	// 			console.log(texture)
    //         },
	// 		null, // onProgress
    //         error => console.log("error",path) // onError
    //     )
	// 	//THREE.ImageUtils.loadTexture("000874.png", null, function(t) {});
    // }
	updateMap(path0){
		let path="./assets/textures/"+path0
        var scope=this;
		...
		if(assets[path]){
			...
			// assets[path]=THREE.ImageUtils.loadTexture(path, null, function(t) {});
			
			// assets[path] = new THREE.TextureLoader().load(path);
			// scope.map=assets[path]
			// scope.map.needsUpdate=true
