...
export class CrowdLod {
    ...
    TraverseBVH(root, frustum, Array) {
        ...
        if (root.prev != null) {
            this.TraverseBVH(root.prev, frustum, Array);
        }
        if (root.next != null) {
            this.TraverseBVH(root.next, frustum, Array);
        }
        if (root.prev == null && root.next == null) {
            Array.push(root.contentArray[0]);
        }
    }

    static frustumCulling(scope) { //每帧执行一次
        var cameraState = scope.getCameraState()
        if (cameraState == scope.cameraStatePreFrame)//相机停止了运动
            if (cameraState !== scope.cameraStatePre) {
                ...
                // 确定BVH的可视性
                var arrayList = []
                scope.TraverseBVH(scope.bvh, scope.frustum, arrayList)
                ...
                //     for (let j = 0; j < scope.frustum.planes.length; j++) {//遍历4个视锥面
