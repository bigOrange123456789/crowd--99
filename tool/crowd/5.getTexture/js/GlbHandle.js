...
GlbHandle.prototype={
    process:function (opt) {
        ...
        this.resourceManager.meshs=[]
        glb.scene.traverse(node=>{
            if(node instanceof THREE.Mesh){
                this.resourceManager.meshs.push(node)
            }
        })
    ...
    downloadMaterial:(scope)=>{
        let meshArr=scope.resourceManager.meshs;
        let index=0
        var myInterval=setInterval(function () {
            //下载材质
            let mesh=meshArr[index]
            scope.download.materialDownload(//下载说明信息
                mesh.material,//scope.resourceManager.resourceInfoGet(),
                mesh.name+".json"
            );
            index++;
            if(index>=meshArr.length){
                clearInterval(myInterval);
            }
        },1000);//有时不需要下载也要等待，这需要优化
    },
