...
class Sphere:
    ...
    def distance(data,centers,distance_group):
        ...
        return dist
    @staticmethod
    def distance_center0(data,index,class0,distance_group):#class0是该类上次迭代的结果
        arr=[]
        for i in range(len(class0)):
            if index>=class0[i]:
                ...
                a=class0[i]
                b=index
            if USE_distance_group:
                arr.append(
                    distance_group[int((a+1)*a/2+b)]
                )
            else:
                arr.append(
                    Sphere.distance_group(data[a],data[b])
                )
...
class Clustering:#目前使用欧式距离
    ...
    def kMeans(self,dataSet, step):
        ...
        if USE_distance_group:
            distance_group=np.zeros( int( (data.shape[0]+1)*data.shape[0]/2 ) )
            for i in range(data.shape[0]):
                for j in range(data.shape[0]):
                    if i>=j:
                        # distance_group[i*data.shape[0]+j]
                        distance_group[int((i+1)*i/2+j)]=Sphere.distance_group(dataSet[i],dataSet[j])
